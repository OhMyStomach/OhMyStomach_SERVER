package org.ohmystomach.ohmystomach_server.domain.smokeReview.application;

import lombok.RequiredArgsConstructor;
import org.ohmystomach.ohmystomach_server.domain.smoke.application.SmokeService;
import org.ohmystomach.ohmystomach_server.domain.smokeReview.dao.SmokeReviewRepository;
...
public class SmokeReviewService {
   ...
   */
  public ApiResponse<List<SmokeReview>> getReviewsBySmokeId(Long smokeId, String sort) {
    List<SmokeReview> reviewList;
    if ("ratingDesc".equalsIgnoreCase(sort)) {
      reviewList = reviewRepository.findBySmokeIdOrderByRatingDesc(smokeId);
    } else if ("ratingAsc".equalsIgnoreCase(sort)) {
      reviewList = reviewRepository.findBySmokeIdOrderByRatingAsc(smokeId);
    } else {
      reviewList = reviewRepository.findBySmokeIdOrderByCreatedDateDesc(smokeId);
    }
    if(reviewList.isEmpty()) {
      return ApiResponse.ok("후기가 존재하지 않습니다.");
    }
    return ApiResponse.ok("흡연구역 후기 목록을 성공적으로 조회했습니다.", reviewList);
  }
